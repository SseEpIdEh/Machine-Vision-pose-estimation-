...
cap = cv2.VideoCapture('C:/Users/Mehrdad/PycharmProjects/Files and codes/clips/4.mp4')

E_pose = mp.solutions.pose
Pose = E_pose.Pose()
Draw = mp.solutions.drawing_utils
blank = np.zeros((600, 1200, 3))  # showing in a blank screen
...
while True:
    ...
    if output.pose_landmarks:
        Draw.draw_landmarks(frame, output.pose_landmarks, E_pose.POSE_CONNECTIONS)
        Draw.draw_landmarks(blank, output.pose_landmarks, E_pose.POSE_CONNECTIONS)
        ...
        # print(landmarklist[5])  #number is optional
        # cv2.circle(frame,(landmarklist[13][1], landmarklist[13][2]) ,15,purple,-1)
        ...
        if abs(y13 - y11) < 15 and abs(y14 - y12) < 15 and 0<m1<1.5 :
            cv2.line(frame, (x15, y15), (x13, y13), blue, 4)
            cv2.line(frame, (x13, y13), (x11, y11), blue, 4)
            cv2.line(frame, (x11, y11), (x12, y12), blue, 4)
            cv2.line(frame, (x12, y12), (x14, y14), blue, 4)
            cv2.line(frame, (x14, y14), (x16, y16), blue, 4)
            cv2.line(frame, (cx1, cy1), (cx2, cy2), blue, 4)
            cv2.putText(blank, "T Shape", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, blue, 3)
            print("T")
        ...
        if abs(y13 + 25<y11) and abs(y14 +25 <y12 and 3<m2<4):
            ...
            cv2.putText(blank, "Y Shape", (20, 100), cv2.FONT_HERSHEY_SIMPLEX,1, blue, 3)
            print('y')

    cv2.imshow("Pose", frame)
    cv2.imshow("Blank", blank)
    blank = np.zeros((600, 1200, 3))
    cv2.waitKey(1)
